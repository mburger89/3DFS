...
- **Gamepad** (`Input/GameControllerManager.swift`, `GCExtendedGamepad`) is polled by a second 60fps `Timer` in `FileScapeSceneView` (`handleGamepad()`), same pattern as WASD: left stick → `camera.pan`, right stick → `camera.orbit`, triggers → `camera.zoom`, all applied every frame the values are outside the manager's deadzone (so held deflection reads as continuous movement, unlike the discrete `pressedChangedHandler` used for the A/B buttons). It works on every platform GameController supports, including visionOS over Bluetooth. Directory selection has no cursor to click, so on macOS/iOS a fixed screen-center reticle raycasts into the scene each frame (see Aim & hover preview below); pressing A enters the aimed node, B calls `navigator.navigateBack()`. The reticle raycast needs a real camera transform, so it's `#if !os(visionOS)`-gated — on visionOS the existing gaze + pinch `TapGesture` already provides the equivalent "look and select," so the gamepad there only drives camera movement and the back button.
- **Aim & hover preview** (`FileSystemSceneManager`, macOS/iOS only): mouse/trackpad hover (`.onContinuousHover` in `FileScapeSceneView`, converted to a world-space ray via manual NDC→camera-space unprojection using `PerspectiveCameraComponent.fieldOfViewInDegrees`) and the gamepad reticle both feed into the same `setAimed(_:index:)`. Aiming at a directory halves its box (`Entity.move(to:relativeTo:duration:timingFunction:)` animates `transform.scale.y` to 0.5 with `translation.y` adjusted so the base stays flush with the floor) and stacks a shrunk (`0.42×` uniform scale) replica of that folder's own children on top of it — a miniature version of the main grid, built by scanning the folder (`FileSystemScanner.scan`) and reusing `VolumeNode.make` per child with `CollisionComponent`/`InputTargetComponent` stripped so the preview itself isn't raycastable. `loadGrid` calls `clearAim()` up front so stale previews don't survive a navigation or theme change.
